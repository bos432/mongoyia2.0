...
class m260623_095000_mongoyia_customer_service_analytics_permission extends Migration
{
    private $permissions = [
        5901 => [
            'parent_id' => 5853,
            'name' => '客服深度统计',
            'path' => '/mall/kf/analytics',
            'icon' => '',
            'level' => 4,
            'sort' => 76,
        ],
        ...
        foreach ($this->permissions as $id => $item) {
            ...
            if (!$exists) {
                $this->insert('{{%base_permission}}', [
                    ...
                    'name' => $item['name'],
                    'app_id' => 'backend',
                    'brief' => 'Phase 9 customer-service deep analytics dashboard and CSV export',
                    'path' => $item['path'],
                    'icon' => $item['icon'],
                    'tree' => '',
                    'level' => $item['level'],
