...
class m260621_161000_mongoyia_customer_service_chat_ticket_permission extends Migration
{
    private $permissions = [
        5889 => [
            'parent_id' => 5853,
            'name' => '客服聊天建工单',
            'path' => '/mall/kf/ticket-create-from-session',
            'icon' => '',
            'level' => 4,
            'sort' => 64,
        ],
        ...
        foreach ($this->permissions as $id => $item) {
            ...
            if (!$exists) {
                $this->insert('{{%base_permission}}', [
                    ...
                    'name' => $item['name'],
                    'app_id' => 'backend',
                    'brief' => 'Phase 8 customer-service chat workbench ticket create API',
                    'path' => $item['path'],
                    'icon' => $item['icon'],
                    'tree' => '',
                    'level' => $item['level'],
