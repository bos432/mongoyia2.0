...
class CategoryController extends BaseController
{
    ...
    public function actionView()
    {
        ...
        if (!is_null($keyword)) {
            if (strlen($keyword) > 0) {
                $productIds = ArrayHelper::getColumn(Lang::find()->where(['table_code' => Product::getTableCode()])->andWhere(['like', 'content', $keyword])->all(), 'target_id');

                SearchLog::create($keyword);
            } else {
                $productIds = ArrayHelper::getColumn(MallPlatformHelper::productQuery()->all(), 'id');
            }
        } else {
            ...
            if ($id) {
                ...
                $model = Category::findOne(['seo_url' => $seoUrl]);
            }
        ...
        $query = MallPlatformHelper::productQuery();
        ...
        $brandFilter = Brand::find()->where(['status' => Brand::STATUS_ACTIVE, 'id' => $brandIds])->all();
