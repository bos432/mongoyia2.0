<?php

namespace console\controllers;

use common\services\mall\MongoyiaProductionEvidenceSummaryService;
use Yii;
use yii\console\Controller;
use yii\console\ExitCode;

class MongoyiaProductionEvidenceSummaryController extends Controller
{
    public $evidenceDir = 'runtime/handover';
    public $acceptanceDir = 'runtime/acceptance';
    public $outputDir = '';
    public $fixture = false;
    public $strict = false;
    public $failOnPending = false;

    private $failures = 0;
    private $warnings = 0;

    public function options($actionID)
    {
        return array_merge(parent::options($actionID), [
            'evidenceDir',
            'acceptanceDir',
            'outputDir',
            'fixture',
            'strict',
            'failOnPending',
        ]);
    }

    public function actionRun()
    {
        $this->stdout("Mongoyia production evidence summary\n");
        $this->checkFiles();

        if ($this->fixture) {
            $this->runFixture();
        } else {
            $report = $this->service()->run($this->input());
            $this->recordReportIssues($report);
            $paths = $this->writeExport($report, false);
            $this->stdout("Markdown: {$paths['md']}\nCSV: {$paths['csv']}\n");
        }

        $this->stdout("\nSummary: {$this->failures} failure(s), {$this->warnings} warning(s).\n");
        if ($this->failures > 0 || ($this->strict && $this->warnings > 0)) {
            return ExitCode::UNSPECIFIED_ERROR;
        }

        return ExitCode::OK;
    }

    private function checkFiles(): void
    {
        $this->section('Files');
        $this->requireFileContains('common/services/mall/MongoyiaProductionEvidenceSummaryService.php', [
            'class MongoyiaProductionEvidenceSummaryService',
            'MONGOYIA_PRODUCTION_EVIDENCE_SUMMARY_V1',
            'production_evidence_summary_read_only_index',
            'This summary is read-only',
        ]);
        $this->requireFileContains('console/controllers/MongoyiaProductionEvidenceSummaryController.php', [
            'class MongoyiaProductionEvidenceSummaryController',
            'Mongoyia production evidence summary',
            'Rollback-clean fixture',
        ]);
        $this->requireFileContains('docs/mongoyia-production-evidence-summary.md', [
            'Mongoyia Production Evidence Summary',
            'MONGOYIA_PRODUCTION_EVIDENCE_SUMMARY_V1',
            'mongoyia-production-evidence-summary/run',
        ]);
        $this->requireFileContains('console/controllers/MongoyiaPackageCheckController.php', [
            'MongoyiaProductionEvidenceSummaryController.php',
            'MongoyiaProductionEvidenceSummaryService.php',
        ]);
        $this->requireFileContains('console/controllers/MongoyiaDeliveryIndexController.php', [
            'productionEvidenceSummaryPath',
            'mongoyia-production-evidence-summary-*.md',
            'Production evidence summary result',
        ]);
        $this->requireFileContains('docs/mongoyia-package-index.md', [
            'mongoyia-production-evidence-summary/run',
            'mongoyia-production-evidence-summary-*.md',
        ]);
    }

    private function runFixture(): void
    {
        $this->section('Rollback-clean fixture');
        try {
            $businessCounts = $this->businessTableCounts();
            $report = $this->service()->run($this->input());

            $this->assertReportValueIn($report, 'result', ['WARN', 'FAIL'], 'Production evidence summary remains non-PASS while production evidence is incomplete.');
            $this->assertTotal($report, 'evidence_row_count', 16, 'Production evidence summary has sixteen rows.');
            $this->assertTotalAtLeast($report, 'pending_count', 1, 'Production evidence summary keeps pending rows before owner signoff.');
            $this->assertTotal($report, 'ready_for_go_live_gate', 0, 'Production evidence summary is not ready for go-live gate by default.');
            $this->assertTotal($report, 'dry_run_network_call_count', 0, 'Production evidence summary does not call external services.');
            $this->assertTotal($report, 'dry_run_write_count', 0, 'Production evidence summary does not write business rows.');
            $this->assertRowExists($report, 'formal_load_test', 'Formal load-test evidence row exists.');
            $this->assertRowExists($report, 'production_external_evidence_review_result_apply', 'Production external evidence review-result apply gate row exists.');
            $this->assertRowExists($report, 'production_external_evidence_final_acceptance', 'Production external evidence final acceptance gate row exists.');
            $this->assertRowExists($report, 'production_launch_signoff_readiness', 'Production launch signoff readiness gate row exists.');
            $this->assertRowExists($report, 'mongolian_review', 'Mongolian review evidence row exists.');

            $paths = $this->writeExport($report, true);
            $this->assertFileContains($paths['md'], [
                '# Mongoyia Production Evidence Summary',
                '- Result:',
                'MONGOYIA_PRODUCTION_EVIDENCE_SUMMARY_V1',
                'ready_for_go_live_gate',
            ]);
            $this->assertFileContains($paths['csv'], [
                'key,gate,status,evidence,reference,owner,notes',
                'formal_load_test',
                'production_external_evidence_review_result_apply',
                'production_external_evidence_final_acceptance',
                'production_launch_signoff_readiness',
                'mongolian_review',
            ]);
            $this->assertBusinessCountsUnchanged($businessCounts);
            $this->ok('Production evidence summary generated read-only pending evidence.');
        } catch (\Throwable $e) {
            $this->fail('Production evidence summary fixture failed: ' . $e->getMessage());
        }
    }

    private function input(): array
    {
        return [
            'evidenceDir' => (string)$this->evidenceDir,
            'acceptanceDir' => (string)$this->acceptanceDir,
            'failOnPending' => (bool)$this->failOnPending,
        ];
    }

    private function writeExport(array $report, bool $fixture): array
    {
        $dir = (string)$this->outputDir !== ''
            ? Yii::getAlias((string)$this->outputDir)
            : dirname(__DIR__, 2) . DIRECTORY_SEPARATOR . 'runtime' . DIRECTORY_SEPARATOR . 'handover';
        if (!is_dir($dir)) {
            mkdir($dir, 0775, true);
        }

        $stamp = date('Ymd-His') . ($fixture ? '-fixture-' . mt_rand(1000, 9999) : '');
        $base = $dir . DIRECTORY_SEPARATOR . 'mongoyia-production-evidence-summary-' . $stamp;
        $md = $base . '.md';
        $csv = $base . '.csv';
        $service = $this->service();
        file_put_contents($md, implode("\n", $service->markdownLines($report)) . "\n");
        file_put_contents($csv, implode("\n", $service->csvLines($report)) . "\n");

        return ['md' => $md, 'csv' => $csv];
    }

    private function service(): MongoyiaProductionEvidenceSummaryService
    {
        return new MongoyiaProductionEvidenceSummaryService(dirname(__DIR__, 2));
    }

    private function recordReportIssues(array $report): void
    {
        if (($report['result'] ?? '') === 'FAIL') {
            foreach (($report['issues'] ?? []) as $issue) {
                $this->fail((string)$issue);
            }
            return;
        }

        foreach (($report['issues'] ?? []) as $issue) {
            $this->warn((string)$issue);
        }
        if ($this->failOnPending && (int)($report['totals']['pending_count'] ?? 0) > 0) {
            $this->fail('Production evidence summary has pending rows and failOnPending=1.');
        }
    }

    private function businessTableCounts(): array
    {
        $counts = [];
        foreach ([
            '{{%mall_order}}',
            '{{%mall_order_product}}',
            '{{%mall_payment_attempt}}',
            '{{%base_message}}',
            '{{%chat_message}}',
            '{{%base_fund_log}}',
            '{{%mall_customer_service_ticket}}',
            '{{%mall_customer_service_event}}',
            '{{%mall_customer_service_stat_daily}}',
        ] as $table) {
            if (Yii::$app->db->schema->getTableSchema($table, true) === null) {
                continue;
            }
            $counts[$table] = (int)(new \yii\db\Query())->from($table)->count('*', Yii::$app->db);
        }

        return $counts;
    }

    private function assertBusinessCountsUnchanged(array $before): void
    {
        foreach ($before as $table => $expected) {
            $actual = (int)(new \yii\db\Query())->from($table)->count('*', Yii::$app->db);
            if ($actual !== $expected) {
                $this->fail("Business table {$table} changed. Expected {$expected}, got {$actual}.");
                return;
            }
        }
        $this->ok('Orders, payments, chats, files, funds, tickets, and statistics were not mutated by production evidence summary.');
    }

    private function assertReportValue(array $report, string $key, string $expected, string $message): void
    {
        $actual = (string)($report[$key] ?? '');
        if ($actual !== $expected) {
            $this->fail("{$message} Expected {$expected}, got {$actual}.");
            return;
        }
        $this->ok($message);
    }

    private function assertReportValueIn(array $report, string $key, array $expectedValues, string $message): void
    {
        $actual = (string)($report[$key] ?? '');
        if (!in_array($actual, $expectedValues, true)) {
            $this->fail("{$message} Expected one of " . implode(', ', $expectedValues) . ", got {$actual}.");
            return;
        }
        $this->ok($message);
    }

    private function assertTotal(array $report, string $key, int $expected, string $message): void
    {
        $actual = (int)($report['totals'][$key] ?? -1);
        if ($actual !== $expected) {
            $this->fail("{$message} Expected {$expected}, got {$actual}.");
            return;
        }
        $this->ok($message);
    }

    private function assertTotalAtLeast(array $report, string $key, int $minimum, string $message): void
    {
        $actual = (int)($report['totals'][$key] ?? -1);
        if ($actual < $minimum) {
            $this->fail("{$message} Expected at least {$minimum}, got {$actual}.");
            return;
        }
        $this->ok($message);
    }

    private function assertRowExists(array $report, string $key, string $message): void
    {
        foreach (($report['rows'] ?? []) as $row) {
            if ((string)$row['key'] === $key) {
                $this->ok($message);
                return;
            }
        }
        $this->fail("{$message} Row {$key} missing.");
    }

    private function requireFileContains(string $path, array $needles): void
    {
        $fullPath = Yii::getAlias('@app') . '/../' . $path;
        if (!is_file($fullPath)) {
            $this->fail("Missing file {$path}.");
            return;
        }
        $content = (string)file_get_contents($fullPath);
        foreach ($needles as $needle) {
            if (strpos($content, $needle) === false) {
                $this->fail("File {$path} missing '{$needle}'.");
                return;
            }
        }
        $this->ok("File contains required markers: {$path}");
    }

    private function assertFileContains(string $path, array $needles): void
    {
        if (!is_file($path)) {
            $this->fail("Missing export file {$path}.");
            return;
        }
        $content = (string)file_get_contents($path);
        foreach ($needles as $needle) {
            if (strpos($content, $needle) === false) {
                $this->fail("Export file {$path} missing '{$needle}'.");
                return;
            }
        }
        $this->ok("Export file contains required markers: {$path}");
    }

    private function section(string $name): void
    {
        $this->stdout("\n[{$name}]\n");
    }

    private function ok(string $message): void
    {
        $this->stdout("OK   {$message}\n");
    }

    private function warn(string $message): void
    {
        $this->warnings++;
        $this->stdout("WARN {$message}\n");
    }

    private function fail(string $message): void
    {
        $this->failures++;
        $this->stderr("FAIL {$message}\n");
    }
}
