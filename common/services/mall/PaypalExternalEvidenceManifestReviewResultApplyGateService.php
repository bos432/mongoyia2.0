...
{
    public const GATE_VERSION = 'MONGOYIA_PAYPAL_EXTERNAL_EVIDENCE_MANIFEST_REVIEW_RESULT_APPLY_GATE_V1';

    private const MODE = 'paypal_external_evidence_manifest_review_result_apply_gate_read_only_no_persistence_no_artifact_access';
    ...
    public function run(): array
    {
        ...
        $preconditions = [
            $this->precondition(
                ...
                'PAYPAL_ENABLED must remain false while external manifest review-result apply is only a gate.',
                $this->envBool('PAYPAL_ENABLED', false) ? 'PAYPAL_ENABLED=true' : 'PAYPAL_ENABLED=false'
            ),
            $this->manifestReviewSignoffImportDryRunPrecondition(),
            $this->documentationPrecondition(),
            $this->reviewResultApplyContractPrecondition($planRows),
        ...
        return [
            ...
            'sourceGateVersions' => [
                PaypalExternalEvidenceManifestReviewSignoffImportDryRunService::GATE_VERSION,
            ],
            'mode' => self::MODE,
            'runtimeEnabled' => $this->envBool('PAYPAL_ENABLED', false),
            'manifestReviewResultValid' => empty($issues),
            'manifestReviewResultApplyAllowed' => false,
            'manifestReviewResultApplyExecuted' => false,
    ...
    private function reviewRows(): array
    {
        $base = [
            ...
            'review_result_ref' => 'review-result:PAYPAL-EXT-001',
        ...
        return [
            array_merge($base, [
                ...
                'result_reason' => 'Business reviewer approved the sanitized external manifest review.',
            ...
            array_merge($base, [
                ...
                'decision' => 'approve',
                'result_status' => 'ready_for_external_apply',
                'result_reason' => 'Security reviewer approved the redaction and artifact-reference boundary.',
            ...
            array_merge($base, [
                ...
                'decision' => 'approve',
                'result_status' => 'ready_for_external_apply',
                'result_reason' => 'Technical reviewer approved manifest, cleanup, and ticket traceability.',
    ...
    private function applyPlanRows(array $rows): array
    {
        ...
        return [
            [
                'manifest_review_id' => 'manifest-review:PAYPAL-EXT-001',
                'operation' => 'would_record_manifest_review_result',
                'source_review_result_ref' => 'review-result:PAYPAL-EXT-001',
                'approved_role_count' => count($approvedRoles),
                'required_role_count' => 3,
                'manifest_review_result_valid' => count($approvedRoles) === 3,
                'apply_allowed' => false,
                'apply_executed' => false,
                'manifest_review_accepted' => false,
                'evidence_bundle_accepted' => false,
                'paypal_enablement_allowed' => false,
                'reason' => 'Read-only gate: real review-result apply remains external and manually approved.',
            ],
        ];
    }
    ...
    private function validatePlanRows(array $rows): array
    {
        ...
        foreach ($rows as $index => $row) {
            if (($row['operation'] ?? '') !== 'would_record_manifest_review_result') {
                $issues[] = 'plan_' . $index . '_invalid_operation';
            }
            foreach (['apply_allowed', 'apply_executed', 'manifest_review_accepted', 'evidence_bundle_accepted', 'paypal_enablement_allowed'] as $key) {
    ...
    private function manifestReviewSignoffImportDryRunPrecondition(): array
    {
        $path = $this->latestHandoverFile('mongoyia-payment-provider-paypal-external-evidence-manifest-review-signoff-import-dry-run-*.md');
        $result = $this->readReportResult($path);
        $signoffReport = (new PaypalExternalEvidenceManifestReviewSignoffImportDryRunService($this->rootPath))->run();
        $inputValid = (bool)($signoffReport['manifestReviewSignoffInputValid'] ?? false);
        $applied = (bool)($signoffReport['manifestReviewSignoffImportApplied'] ?? true);
        $accepted = (bool)($signoffReport['manifestReviewAccepted'] ?? true);
        $evidenceAccepted = (bool)($signoffReport['evidenceBundleAccepted'] ?? true);
        $enablementAllowed = (bool)($signoffReport['paypalEnablementAllowed'] ?? true);
        $ok = $path !== '' && $result === 'PASS' && $inputValid && !$applied && !$accepted && !$evidenceAccepted && !$enablementAllowed;

        return $this->precondition(
            'manifest_review_signoff_import_dry_run_report',
            $ok,
            $ok ? 'pass' : 'blocked',
            'The PayPal external manifest review signoff import dry-run must PASS, while no import, acceptance, or enablement is applied.',
            $ok ? $this->relativePath($path) : 'Missing/non-PASS manifest review signoff import dry-run report or disabled flags are not in the expected state.'
        );
    }
    ...
    private function documentationPrecondition(): array
    {
        ...
        return $this->precondition(
            'manifest_review_result_apply_documentation',
            empty($missing),
            empty($missing) ? 'ready' : 'blocked',
            'Payment provider docs must describe the external manifest review-result apply gate.',
            empty($missing) ? 'External manifest review-result apply gate documentation markers are present.' : 'Missing markers: ' . implode(', ', $missing)
        );
    }

    private function reviewResultApplyContractPrecondition(array $planRows): array
    {
        $row = $planRows[0] ?? [];
        $ok = count($planRows) === 1
            && (string)($row['operation'] ?? '') === 'would_record_manifest_review_result'
            && (int)($row['approved_role_count'] ?? 0) === 3
            && !((bool)($row['apply_allowed'] ?? true))
            && !((bool)($row['apply_executed'] ?? true))
            && !((bool)($row['manifest_review_accepted'] ?? true))
            && !((bool)($row['evidence_bundle_accepted'] ?? true))
            && !((bool)($row['paypal_enablement_allowed'] ?? true));

        return $this->precondition(
            'manifest_review_result_apply_contract',
            $ok,
            $ok ? 'ready' : 'blocked',
            'The apply gate must produce a single dry-run plan row and keep all apply/acceptance/enablement flags false.',
            $ok ? 'External manifest review-result apply contract is read-only and disabled.' : 'External manifest review-result apply plan does not match the expected disabled contract.'
        );
    }
    ...
    {
        return $this->precondition(
            'manifest_review_result_fixture_rows',
            empty($rowIssues),
    ...
    private function acceptanceWiringPrecondition(): array
    {
        ...
        return $this->precondition(
            ...
            empty($missing),
            empty($missing) ? 'ready' : 'blocked',
            'Acceptance must include the external manifest review-result apply gate after signoff import and before live enablement.',
    ...
    private function uiHiddenPrecondition(): array
    {
        ...
        return $this->precondition(
            ...
            'PayPal UI controls must stay hidden while external manifest review-result apply is only a gate.',
    ...
    private function providerApiBoundaryPrecondition(): array
    {
        ...
        return $this->precondition(
            ...
            'PaymentController must not contain live PayPal API URLs or credential reads during external manifest review-result apply gate.',
    ...
    private function totals(array $rows, array $planRows, array $preconditions, array $rowIssues, array $planIssues): array
    {
        ...
        return [
            ...
            'manifest_review_result_valid' => $valid ? 1 : 0,
            'manifest_review_result_apply_allowed' => 0,
            'manifest_review_result_apply_executed' => 0,
            'manifest_review_accepted' => 0,
    ...
    private function gateChecks(array $preconditions, array $rowIssues, array $planIssues): array
    {
        ...
        $issues = array_merge($rowIssues, $planIssues);
        $checks[] = [
            'key' => 'manifest_review_result_valid',
        ...
        $checks[] = [
            'key' => 'manifest_review_result_apply',
            'status' => 'disabled',
            'details' => 'No manifest review result is persisted or applied by this gate.',
        ];
        $checks[] = [
            'key' => 'manifest_review_acceptance',
            'status' => 'pending',
            'details' => 'Manifest review acceptance remains pending until real sanitized evidence and external approvals are supplied.',
        ];
        $checks[] = [
            'key' => 'evidence_bundle_acceptance',
            'status' => 'pending',
            'details' => 'The evidence bundle remains unaccepted until real sanitized evidence and approvals are externally accepted.',
        ];
        $checks[] = [
    ...
    public function markdownLines(array $report): array
    {
        $lines = [
            ...
            '- Manifest review result valid: ' . (($report['manifestReviewResultValid'] ?? false) ? 'yes' : 'no'),
            '- Manifest review result apply allowed: ' . (($report['manifestReviewResultApplyAllowed'] ?? true) ? 'yes' : 'no'),
            '- Manifest review result apply executed: ' . (($report['manifestReviewResultApplyExecuted'] ?? true) ? 'yes' : 'no'),
        ...
        $lines = array_merge($lines, [
            ...
            '## Review Result Rows',
            '',
            '| Review id | Role | Decision | Result status | Review result ref | Cleanup ref |',
            '|---|---|---|---|---|---|',
        ]);
        ...
        $lines = array_merge($lines, [
            ...
            '## Dry-Run Apply Plan',
            '',
            '| Review id | Operation | Approved roles | Required roles | Apply allowed | Apply executed | Review accepted | Evidence accepted | PayPal allowed | Reason |',
            '|---|---|---:|---:|---:|---:|---:|---:|---:|---|',
        ]);
        foreach (($report['applyPlanRows'] ?? []) as $row) {
            $lines[] = '| ' . $this->escapeCell((string)$row['manifest_review_id'])
                ...
                . ' | ' . ((bool)$row['manifest_review_accepted'] ? '1' : '0')
        ...
        return array_merge($lines, [
            ...
            '## Boundaries',
            '',
            '- manifest_review_result_valid=1 means the sanitized review-result metadata is valid, not that any result was applied.',
            '- manifest_review_result_apply_allowed=0 remains intentional; this gate cannot approve persistence.',
            '- manifest_review_result_apply_executed=0 remains intentional; this gate never writes reviewer/signoff/review rows.',
    ...
    {
        $lines = ['manifest_review_id,review_result_ref,test_host,manifest_ref,cleanup_ref,ticket_ref,artifact_hash_ref,reviewer_role,decision,result_status,result_reason,reviewed_at'];
        foreach (($report['reviewRows'] ?? []) as $row) {
            $lines[] = implode(',', [
