        ...
        return [
            'access' => [
                ...
                'rules' => [
                    ...
                    [
                        'actions' => ['logout', 'switch-login', 'index', 'info', 'log', 'change-password', 'me', 'clear-cache', 'stat', 'statt', 'color'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
...
//                        return $this->redirect(CommonHelper::getHostPrefix($store->host_name) . '/backend/site/login-backend?token=' . $user->token);
    ...
    public function actionInfo()
    {
        ...
        $product_visit =  (new \yii\db\Query)
            ->from('fb_mall_product_visit')
            ->select(['count(*) as count'])
//            ->where(['between', 'time', $startTime, $endTime])
            ->count();
        $startTime = strtotime(date('Y-m-d').' 00:00:00');
        $endTime = strtotime(date('Y-m-d').' 23:59:59');
        $product_visit_today =  (new \yii\db\Query)
            ->from('fb_mall_product_visit')
            ->select(['count(*) as count'])
            ->where(['between', 'time', $startTime, $endTime])
            ->count();
        ...
        return $this->render($this->action->id, [
            ...
            'product_visit_today'=>$product_visit_today
        ]);
    }
...
//                    var_dump(Yii::$app->authSystem->isBackend());
//                    exit();
                    // 非管理员不能登录后台
                    if (!Yii::$app->authSystem->isBackend()) {
                        Yii::$app->logSystem->login(Yii::$app->user->identity->username ?? '' . ' ' . Yii::$app->user->id . ' with no auth to backend', null, true);
...
//                            }
//                        } else {
//                        var_dump(Yii::$app->authSystem->isAdmin());exit();
                            return Yii::$app->authSystem->isAdmin() ? $this->redirect(['/']) : $this->goBack();
//                        }
                    }
                }
    ...
    /**
     * Render a same-origin POST logout bridge for browser/user account switching.
     *
    ...
    public function actionSwitchLogin()
    {
        ...
        $this->layout = false;
        return $this->render('switch-login');
    }
