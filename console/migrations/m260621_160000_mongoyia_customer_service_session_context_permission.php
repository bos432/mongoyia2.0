...
class m260621_160000_mongoyia_customer_service_session_context_permission extends Migration
{
    private $permissions = [
        5888 => [
            'parent_id' => 5853,
            'name' => '客服会话上下文',
            'path' => '/mall/kf/session-context',
            'icon' => '',
            'level' => 4,
            'sort' => 63,
        ],
        ...
        foreach ($this->permissions as $id => $item) {
            ...
            if (!$exists) {
                $this->insert('{{%base_permission}}', [
                    ...
                    'name' => $item['name'],
                    'app_id' => 'backend',
                    'brief' => 'Phase 8 customer-service session context readonly API',
                    'path' => $item['path'],
                    'icon' => $item['icon'],
                    'tree' => '',
                    'level' => $item['level'],
