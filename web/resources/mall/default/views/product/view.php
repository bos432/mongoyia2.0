                    ...
                    <ul id="product-attribute">
                        ...
                        <li>
                            ...
                            <div id="attribute-<?= $attribute->id; ?>" class="attribute-btn attribute-btn-<?= $attribute['type'] ?>">
                                <?php foreach ($attribute->attributeItems as $key => $attributeItem) { $checked = in_array((int)$attributeItem['id'], $arrDefaultAttribute); ?>
                                    <?php if ($attribute['type'] == Attribute::TYPE_COLOR) { ?>
                                        <label for="attribute-item-<?= $attributeItem['id'] ?>" class="<?= $checked ? 'active' : '' ?>">
                                            <input type="radio" name="attribute-item-<?= $attributeItem['attribute_id'] ?>" id="attribute-item-<?= $attributeItem['id'] ?>" <?= $checked ? 'checked="checked"' : '' ?> value="<?= $attributeItem['id'] ?>">
                                            <?php $attributeItemLabel = $mapProductAttributeItemAttributeItemIdLabel[$attributeItem['id']] ?? ''; ?>
                                            <span class="checkmark" style="background-color: #<?= $attributeItemLabel != '' ? $attributeItemLabel : '000' ?>"></span>
                                        </label>
                                    <?php } elseif ($attribute['type'] == 3) { ?>
                                        <label for="attribute-item-<?= $attributeItem['id'] ?>" class="<?= $checked ? 'active' : '' ?>" style="background-image: url(<?= $mapProductAttributeItemAttributeItemIdLabel[$attributeItem['id']] ?? ImageHelper::get('/resources/images/add-sku.png'); ?>)">
                                            <input type="radio" name="attribute-item-<?= $attributeItem['attribute_id'] ?>" id="attribute-item-<?= $attributeItem['id'] ?>" <?= $checked ? 'checked="checked"' : '' ?> value="<?= $attributeItem['id'] ?>">
                                        </label>
                                    <?php } else { ?>
                                        <label for="attribute-item-<?= $attributeItem['id'] ?>" class="<?= $checked ? 'active' : '' ?>">
...
<!--                        <li><b>--><?php //= Yii::t('mall', 'Distribution') ?><!--</b>-->
<!--                            <img src="https://api.2dcode.biz/v1/create-qr-code?data=--><?php //=$url?><!--" alt="">-->
<!--                        </li>-->
                    </ul>
...
<!--                            <a class="nav-link" id="nav-consultation-tab" data-toggle="tab" href="#nav-consultation" role="tab" aria-controls="nav-consultation" aria-selected="false">--><?php //= Yii::t('mall', 'Consultations') ?><!--</a>-->
                        </div>
                    </nav>
...
<!--                            <h3 class="content-param">--><?php //= Yii::t('mall', 'Product Params') ?><!--</h3>-->
<!--                            <table class="table table-bordered">-->
<!--                                --><?php //if (is_array($allParams)) { foreach ($allParams as $child2) { ?>
<!--                                    <tr><td rowspan="--><?php //= count($child2->children) == 0 ? 1 : count($child2->children) ?><!--">--><?php //= fbt(Param::getTableCode(), $child2->id, 'name', $child2->name) ?><!--</td>-->
<!--                                    --><?php //$i = 0; if (isset($child2->children) && is_array($child2->children) && count($child2->children)) { foreach ($child2->children as $child3) { ?>
<!--                                        --><?php //if ($i == 0) { ?>
<!--                                            <td>--><?php //= fbt(Param::getTableCode(), $child3->id, 'name', $child3->name) ?><!--<td>--><?php //= Yii::t('param', $mapProductParamIdContent[$child3->id] ?? '-') ?><!--</td>-->
<!--                                        --><?php //} else { ?>
<!--                                            <tr><td>--><?php //= fbt(Param::getTableCode(), $child3->id, 'name', $child3->name) ?><!--<td>--><?php //= Yii::t('param', $mapProductParamIdContent[$child3->id] ?? '-') ?><!--</td></tr>-->
<!--                                        --><?php //} $i++; ?>
<!--                                    --><?php //} } else { ?>
<!--                                        <td width="50%">--><?php //= Yii::t('param', $mapProductParamIdContent[$child2->id] ?? '-') ?><!--</td>-->
<!--                                    --><?php //} ?>
<!--                                    </tr>-->
<!--                                --><?php //} } ?>
<!--                            </table>-->
...
$(document).ready(function() {
    ...
    try {
        ...
        if (objAttributeInfo !== undefined && objAttributeInfo.price !== undefined) {
