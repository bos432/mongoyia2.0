...
{
    public const GATE_VERSION = 'MONGOYIA_PAYPAL_SANDBOX_EVIDENCE_SIGNOFF_IMPORT_DRY_RUN_V1';

    private const MODE = 'paypal_sandbox_evidence_signoff_import_dry_run_no_persistence_no_artifact_access';
    ...
    public function run(): array
    {
        ...
        $preconditions = [
            $this->precondition(
                ...
                'PAYPAL_ENABLED must remain false while signoff import is only a dry-run.',
                $this->envBool('PAYPAL_ENABLED', false) ? 'PAYPAL_ENABLED=true' : 'PAYPAL_ENABLED=false'
            ),
            $this->bundleReviewSignoffGatePrecondition(),
        ...
        return [
            ...
            'sourceGateVersions' => [
                PaypalSandboxEvidenceBundleReviewSignoffGateService::GATE_VERSION,
            ],
            'mode' => self::MODE,
            'runtimeEnabled' => $this->envBool('PAYPAL_ENABLED', false),
            'signoffInputValid' => empty($issues),
    ...
    {
        return [
            $this->field('bundle_id', 'string', 'Sanitized evidence bundle id, not a filename or local path.'),
            $this->field('test_host', 'https_url', 'HTTPS test host that produced the evidence.'),
            $this->field('manifest_ref', 'safe_ref', 'Sanitized manifest reference or ticket id.'),
            ...
            $this->field('reason', 'string', 'Non-sensitive signoff or rejection reason.'),
    ...
    private function fixtureRows(): array
    {
        ...
        return [
            array_merge($base, [
                ...
                'decision' => 'approve',
                'reason' => 'Business scope matches approved sandbox checklist.',
            ...
            array_merge($base, [
                ...
                'decision' => 'approve',
                'reason' => 'Redaction checklist has no open blocking item.',
                'reviewed_at' => '2026-06-19T00:05:00Z',
            ]),
            array_merge($base, [
                'reviewer_role' => 'technical',
                'reviewer_ref' => 'reviewer:technical-owner',
                'decision' => 'approve',
                'reason' => 'Manifest references, hashes, cleanup ref, and host traceability are present.',
                'reviewed_at' => '2026-06-19T00:10:00Z',
            ]),
        ];
    }
    ...
    private function validateRows(array $rows, array $fields): array
    {
        ...
        foreach ($rows as $index => $row) {
            ...
            $roles[] = (string)($row['reviewer_role'] ?? '');
            if (!in_array((string)($row['reviewer_role'] ?? ''), ['business', 'security', 'technical'], true)) {
            ...
            foreach (['bundle_id', 'manifest_ref', 'reviewer_ref', 'cleanup_ref', 'ticket_ref', 'notes', 'reason'] as $safeKey) {
    ...
    private function bundleReviewSignoffGatePrecondition(): array
    {
        $path = $this->latestHandoverFile('mongoyia-payment-provider-paypal-sandbox-evidence-bundle-review-signoff-gate-*.md');
        $result = $this->readReportResult($path);
        $signoffReport = (new PaypalSandboxEvidenceBundleReviewSignoffGateService($this->rootPath))->run();
        $ready = (bool)($signoffReport['bundleReviewSignoffReady'] ?? false);
        $accepted = (bool)($signoffReport['evidenceBundleAccepted'] ?? true);
        $enablementAllowed = (bool)($signoffReport['paypalEnablementAllowed'] ?? true);
        $ok = $path !== '' && $result === 'PASS' && $ready && !$accepted && !$enablementAllowed;

        return $this->precondition(
            'bundle_review_signoff_gate_report',
            $ok,
            $ok ? 'pass' : 'blocked',
            'The PayPal sandbox evidence bundle review signoff gate must PASS, while evidence_bundle_accepted and paypal_enablement_allowed remain 0.',
            $ok ? $this->relativePath($path) : 'Missing/non-PASS bundle review signoff gate report or readiness flags are not in the expected disabled state.'
        );
    }
    ...
    private function documentationPrecondition(): array
    {
        ...
        return $this->precondition(
            'signoff_import_documentation',
            empty($missing),
            empty($missing) ? 'ready' : 'blocked',
            'Payment sandbox evidence documentation must describe the signoff import dry-run.',
            empty($missing) ? 'Signoff import dry-run documentation markers are present.' : 'Missing markers: ' . implode(', ', $missing)
        );
    }
    ...
    private function templateContractPrecondition(array $fields): array
    {
        ...
        return $this->precondition(
            'signoff_import_template_contract',
            $ok,
            $ok ? 'ready' : 'blocked',
            'The signoff import dry-run template must include the required non-sensitive fields.',
    ...
    {
        return $this->precondition(
            'signoff_import_fixture_rows',
            empty($rowIssues),
            empty($rowIssues) ? 'valid' : 'blocked',
            'The dry-run fixture rows must cover business, security, and technical signoff records with safe references.',
    ...
    private function acceptanceWiringPrecondition(): array
    {
        ...
        return $this->precondition(
            ...
            empty($missing),
            empty($missing) ? 'ready' : 'blocked',
            'Acceptance must include the signoff import dry-run after bundle review signoff and before live enablement.',
    ...
    private function uiHiddenPrecondition(): array
    {
        ...
        return $this->precondition(
            ...
            'PayPal UI controls must stay hidden while signoff import is only a dry-run.',
    ...
    private function providerApiBoundaryPrecondition(): array
    {
        ...
        return $this->precondition(
            ...
            'PaymentController must not contain live PayPal API URLs or credential reads during signoff import dry-run.',
    ...
    private function gateChecks(array $preconditions, array $rowIssues): array
    {
        ...
        $checks[] = [
            'key' => 'signoff_input_valid',
            'status' => empty($rowIssues) ? 'ready' : 'blocked',
            'details' => empty($rowIssues) ? 'The sanitized signoff input template and fixture rows are valid.' : 'Row issues: ' . implode(', ', $rowIssues),
        ];
        $checks[] = [
            'key' => 'signoff_import_application',
            'status' => 'disabled',
            'details' => 'No signoff row is persisted by this dry-run command.',
        ];
        $checks[] = [
            'key' => 'evidence_bundle_acceptance',
            'status' => 'pending',
            'details' => 'The evidence bundle remains unaccepted until real sanitized evidence and reviewer signatures are supplied and reviewed externally.',
        ];
        $checks[] = [
    ...
    public function markdownLines(array $report): array
    {
        $lines = [
            ...
            '- Signoff input valid: ' . (($report['signoffInputValid'] ?? false) ? 'yes' : 'no'),
        ...
        $lines = array_merge($lines, [
            ...
            '## Dry-Run Rows',
            '',
            '| Bundle | Role | Decision | Test host | Manifest ref | Cleanup ref |',
            '|---|---|---|---|---|---|',
        ]);
        ...
        return array_merge($lines, [
            ...
            '## Boundaries',
            '',
            '- signoff_input_valid=1 means the sanitized input template is valid, not that any signoff was imported.',
    ...
    {
        $lines = ['bundle_id,test_host,manifest_ref,artifact_hash_ref,reviewer_role,reviewer_ref,decision,reason,reviewed_at,cleanup_ref,ticket_ref,notes'];
        foreach (($report['fixtureRows'] ?? []) as $row) {
            $lines[] = implode(',', [
