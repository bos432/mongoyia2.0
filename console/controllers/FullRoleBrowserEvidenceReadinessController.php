...
class FullRoleBrowserEvidenceReadinessController extends Controller
{
    ...
    private function writeTemplate(): string
    {
        ...
        $lines = [
            ...
            '## Evidence Fill Rules',
            '',
            '- 每个勾选项必须先在右侧浏览器或人工浏览器中实际操作，再把记录写入本文件。',
            '- 记录账号、入口 URL、输入的测试数据摘要、生成或查看到的订单/聊天/工单/素材编号、刷新后的状态。',
            '- 如果某个页面没有数据，也要记录空状态是否正常渲染，不能只写通过。',
            '- 如果发现失败项，保留失败 URL、错误提示、截图或日志路径，并不要勾选对应项。',
            '- 不允许把 OAuth、SMTP、支付、短信、翻译、物流、告警等服务商密钥写进本证据。',
            '',
            '## Safety Boundary',
            '',
            '- [ ] 未触发真实支付成功。',
            ...
            '',
            '- 账号/会话: 待填写。',
            '- 入口 URL: `/backend/`、`/backend/site/info`。',
            '- 测试数据摘要: 待填写。',
            '- [ ] 页面能正常打开: `/backend/`、运营配置、支付统计、通知日志、客服、商品、物流、评论、分销。',
            '- [ ] 运营配置页可查看支付、SMTP、翻译、告警、定时任务、备份、压测、安全、业务签核状态。',
            '- [ ] 支付统计页可查看每日金额、渠道分布、失败原因、回调异常、对账差异或空状态。',
            '- [ ] 通知日志、商品、物流、评论、分销页面的列表和详情入口可用。',
            '- [ ] 登录/注销或入口流程可用。',
            ...
            '## 商家',
            '',
            '- 账号/会话: `zhishichanquan / 123456`。',
            '- 入口 URL: `/backend/site/login` -> `/backend/`。',
            '- 测试数据摘要: 待填写。',
            '- [ ] 使用 `zhishichanquan / 123456` 登录。',
            '- [ ] 页面能正常打开: 商品、订单、发货、物流费用、优惠券、统计、客服隔离。',
            '- [ ] 商品列表、商品编辑/查看、订单列表、订单详情、物流费用页可打开。',
            '- [ ] 优惠券、统计、商家客服入口显示店铺隔离数据或安全空状态。',
            '- [ ] 表单提交、数据保存、列表展示、详情查看正常。',
            '- [ ] 刷新页面后关键数据状态合理。',
            '- [ ] 前端报错/接口报错: 无阻塞性错误。',
            '',
            ...
            '',
            '- 账号/会话: 待填写。',
            '- 入口 URL: `/mall`、`/mall/product/view?id=2`、`/mall/cart/index`、`/mall/chat/index?gid=2`。',
            '- 测试数据摘要: 待填写。',
            '- [ ] 注册/登录或入口流程可用。',
            '- [ ] 页面能正常打开: 首页、分类、搜索、商品详情、购物车、结算、订单、收藏、评论、客服咨询。',
            '- [ ] 加入购物车、提交测试订单、查看订单、收藏、评论、客服消息流程可走通。',
            '- [ ] APP/H5 API 商品详情、购物车、订单、通知、评论接口返回正常业务状态。',
            ...
            '',
            '- 账号/会话: 待填写。',
            '- 入口 URL: `/backend/mall/kf/index`、工单/投诉详情页。',
            '- 测试数据摘要: 待填写。',
            '- [ ] 页面能正常打开: 客服工作台、会话、订单/商品上下文、工单、投诉、满意度。',
            '- [ ] 接收买家消息、回复消息、创建协助单/投诉记录。',
            '- [ ] 可查看买家、订单、商品上下文，只进行协助/记录，不直接改订单、资金、库存。',
            '- [ ] 表单提交、数据保存、列表展示、详情查看正常。',
            '- [ ] 刷新页面后聊天、工单、投诉状态合理。',
            '- [ ] 前端报错/接口报错: 无阻塞性错误。',
            '',
            ...
            '## 移动端视觉',
            '',
            '- 核心页面: 首页、商品详情、购物车、结算/支付页、订单、客服、分销中心、后台客服。',
            ...
            '- 待填写。',
            '',
            '## Final Aggregate Acceptance Command',
            '',
            '```bash',
            ...
            '  --phase10BrowserAccepted=1 \\',
            '  --phase10BrowserEvidencePath=runtime/handover/full-role-browser-evidence.md \\',
            '  --phase11BrowserAccepted=1 \\',
            '  --phase11BrowserEvidencePath=runtime/handover/full-role-browser-evidence.md \\',
            '  --phase12BrowserAccepted=1 \\',
            '  --phase12BrowserEvidencePath=runtime/handover/full-role-browser-evidence.md \\',
            '  --phase13BrowserAccepted=1 \\',
            '  --phase13BrowserEvidencePath=runtime/handover/full-role-browser-evidence.md \\',
            '  --phase14BrowserAccepted=1 \\',
            '  --phase14BrowserEvidencePath=runtime/handover/full-role-browser-evidence.md \\',
            '  --phase15BrowserAccepted=1 \\',
            '  --phase15BrowserEvidencePath=runtime/handover/full-role-browser-evidence.md \\',
            '  --strict=1 \\',
            '  --interactive=0',
            '```',
            '',
