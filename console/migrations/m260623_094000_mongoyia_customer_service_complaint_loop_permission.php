...
class m260623_094000_mongoyia_customer_service_complaint_loop_permission extends Migration
{
    private $permissions = [
        5899 => [
            'parent_id' => 5853,
            'name' => '客服投诉闭环记录',
            'path' => '/mall/kf/complaint-loop-step',
            'icon' => '',
            'level' => 4,
            'sort' => 74,
        ],
        5900 => [
            'parent_id' => 5853,
            'name' => '客服投诉关联协助单',
            'path' => '/mall/kf/complaint-link-assistance',
            'icon' => '',
            'level' => 4,
            'sort' => 75,
        ],
        ...
        foreach ($this->permissions as $id => $item) {
            ...
            if (!$exists) {
                $this->insert('{{%base_permission}}', [
                    ...
                    'name' => $item['name'],
                    'app_id' => 'backend',
                    'brief' => 'Phase 9 customer-service complaint full-loop workflow',
                    'path' => $item['path'],
                    'icon' => $item['icon'],
                    'tree' => '',
                    'level' => $item['level'],
