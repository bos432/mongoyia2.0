...
class CustomerServiceTicketNoteService
{
    public function run(
        ...
        if ($content === '') {
            $result['skipped'][] = ['id' => $ticketId, 'reason' => 'content is required'];
            return $result;
        }
        ...
        try {
            ...
            Yii::$app->db->createCommand()->insert('{{%mall_customer_service_event}}', [
                ...
                'content' => $this->shortText($content, 4000),
                'metadata_json' => json_encode([
                    'source' => 'customer-service-ticket-note',
                    'order_id' => (int)$ticket['order_id'],
                    'product_id' => (int)$ticket['product_id'],
                    'chat_uuid' => (string)$ticket['chat_uuid'],
                ], JSON_UNESCAPED_SLASHES),
                'remark' => 'customer-service ticket note',
                'status' => 1,
                'created_at' => $now,
                'updated_at' => $now,
