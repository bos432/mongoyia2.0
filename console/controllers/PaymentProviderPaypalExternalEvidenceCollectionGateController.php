...
use common\services\mall\PaypalExternalEvidenceCollectionGateService;
use Yii;
use yii\console\Controller;
use yii\console\ExitCode;

class PaymentProviderPaypalExternalEvidenceCollectionGateController extends Controller
{
    public $outputDir = '';
    ...
    {
        $this->stdout("Mongoyia payment provider PayPal external evidence collection gate\n");
        $this->checkFiles();
    ...
    private function checkFiles(): void
    {
        ...
        $this->requireFileContains('common/services/mall/PaypalExternalEvidenceCollectionGateService.php', [
            ...
            'paypal_external_evidence_collection_input_gate_read_only_no_artifact_access',
        ...
        $this->requireFileContains('console/controllers/PaymentProviderPaypalExternalEvidenceCollectionGateController.php', [
            'class PaymentProviderPaypalExternalEvidenceCollectionGateController',
            'Mongoyia payment provider PayPal external evidence collection gate',
        ...
        $this->requireFileContains('console/controllers/MongoyiaPackageCheckController.php', [
            'PaymentProviderPaypalExternalEvidenceCollectionGateController.php',
            'PaypalExternalEvidenceCollectionGateService.php',
        ]);
        $this->requireFileContains('console/controllers/MongoyiaDeliveryIndexController.php', [
            ...
            'mongoyia-payment-provider-paypal-external-evidence-collection-gate-*.md',
            'Payment provider PayPal external evidence collection gate result',
        ]);
    ...
    private function runFixture(): void
    {
        ...
        try {
            ...
            $this->assertTotal($report, 'collection_input_row_count', 9, 'PayPal external collection gate has nine input rows.');
            $this->assertTotal($report, 'valid_collection_input_row_count', 9, 'All PayPal external collection input rows are valid.');
            $this->assertTotal($report, 'required_input_count', 9, 'PayPal external collection gate has nine required inputs.');
            $this->assertTotal($report, 'covered_required_input_count', 9, 'PayPal external collection gate covers all required inputs.');
            $this->assertTotal($report, 'precondition_count', 7, 'PayPal external collection gate has seven preconditions.');
            $this->assertTotal($report, 'satisfied_precondition_count', 7, 'All PayPal external collection local preconditions pass.');
            $this->assertTotal($report, 'pending_external_count', 5, 'PayPal external collection gate keeps five external pending markers.');
            $this->assertTotal($report, 'artifact_read_count', 0, 'PayPal external collection gate does not read artifacts.');
            $this->assertTotal($report, 'artifact_import_count', 0, 'PayPal external collection gate does not import artifacts.');
            $this->assertTotal($report, 'artifact_hash_count', 0, 'PayPal external collection gate does not hash artifacts.');
            $this->assertTotal($report, 'dry_run_network_call_count', 0, 'PayPal external collection gate does not call providers.');
            $this->assertTotal($report, 'dry_run_write_count', 0, 'PayPal external collection gate does not write rows.');
            $this->assertTotal($report, 'collection_input_valid', 1, 'PayPal external collection input metadata is valid.');
            $this->assertTotal($report, 'external_collection_ready', 1, 'PayPal external collection input is locally ready.');
            $this->assertTotal($report, 'external_collection_started', 0, 'PayPal external collection is not started by this gate.');
            ...
            $this->assertGateStatus($report, 'review_result_apply_gate_report', 'pass', 'PayPal review-result apply gate report is PASS.');
            $this->assertGateStatus($report, 'external_collection_documentation', 'ready', 'PayPal external collection documentation is ready.');
            $this->assertGateStatus($report, 'collection_input_contract', 'ready', 'PayPal external collection input contract is ready.');
            $this->assertGateStatus($report, 'acceptance_wiring', 'ready', 'PayPal external collection acceptance wiring is ready.');
            $this->assertGateStatus($report, 'collection_input_valid', 'ready', 'PayPal external collection validation gate is ready.');
            $this->assertGateStatus($report, 'external_collection_start', 'disabled', 'PayPal external collection start remains disabled.');
            ...
            $this->assertFileContains($paths['md'], [
                ...
                '- Result: PASS',
                '- Collection input valid: yes',
                '- External collection ready: yes',
                '- External collection started: no',
                '- Evidence bundle accepted: no',
                '- PayPal enablement allowed: no',
                '| collection_input_valid | 1 |',
                'external_collection_started=0 remains intentional',
            ]);
            $this->assertFileContains($paths['csv'], [
                'input_key,input_type,owner_role,test_host,reference,collection_status,sanitized_only,artifact_access_allowed,provider_call_allowed,write_allowed,required_evidence,notes',
                'test_https_domain,environment,technical,https://test.mongoyia.test,env-ref:PAYPAL-SBX-TEST-DOMAIN',
                'reviewer_signoff_ref,signoff_reference,joint',
            ]);
            $this->assertBusinessCountsUnchanged($businessCounts);
            $this->ok('PayPal external evidence collection gate generated read-only evidence.');
        } catch (\Throwable $e) {
            $this->fail('PayPal external evidence collection gate fixture failed: ' . $e->getMessage());
        }
    }
    ...
    private function writeExport(array $report, bool $fixture): array
    {
        ...
        $base = $dir . DIRECTORY_SEPARATOR . 'mongoyia-payment-provider-paypal-external-evidence-collection-gate-' . $stamp;
        $md = $base . '.md';
        $csv = $base . '.csv';
    ...
    private function service(): PaypalExternalEvidenceCollectionGateService
    {
        return new PaypalExternalEvidenceCollectionGateService(dirname(__DIR__, 2));
    }
    ...
    private function assertBusinessCountsUnchanged(array $before): void
    {
        ...
        $this->ok('Orders, payments, chats, files, funds, tickets, and statistics were not mutated by PayPal external evidence collection gate.');
    }
    ...
    private function requireFileNotContains(string $path, array $needles): void
    {
        ...
        $this->ok("File keeps PayPal external evidence collection boundary disabled: {$path}");
    }
