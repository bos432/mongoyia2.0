...
class BaseController extends \common\components\controller\BaseController
{
    ...
    {
        $mapLangContent = [];
//        var_dump($this->modelClass::getTableCode());
        if ($id) {
            $langItems = Lang::find()
                ->where(['table_code' => $this->modelClass::getTableCode(), 'target_id' => $id])
                ->orderBy(['name' => SORT_ASC])
                ->all();
            foreach ($langItems as $langItem) {
                if(!isset($mapLangContent[$langItem->name . '|' . $langItem->target]) ||
                    $mapLangContent[$langItem->name . '|' . $langItem->target] == ''){
...
//        echo '<pre/>';
//        var_dump($this->modelClass::getLangFieldType());
//        var_dump($lang);
//        var_dump($mapLangContent);
//        exit();
        return $lang;
    }
        ...
        if (isset($post['Lang'])) {
            foreach ($post['Lang'] as $field => $item) {
                ...
                foreach ($post['Lang'][$field] as $target => $content) {
                    ...
                    $lang = Lang::find()->where(['table_code' => $this->modelClass::getTableCode(), 'target_id' => $model->id, 'target' => $target, 'name' => $field])->one();
                    if (!$lang) {
                        $lang = new Lang();
