...
class Uploader
{
    ...
    public function save()
    {
        ...
        $httpPrefix = $this->normalizeHttpPrefix($this->config['httpPrefix'] ?? Yii::getAlias('@attachmentUrl') . '/');
        if (!empty($this->config['takeOverUrl'])) {
            $httpPrefix = $this->normalizeHttpPrefix($this->config['takeOverUrl']);
        }
        ...
        if (!$model) {
            throw new NotFoundHttpException('附件写入数据库失败');
        }
    ...
    protected function normalizeHttpPrefix($httpPrefix)
    {
        $httpPrefix = trim((string)$httpPrefix);
        if ($httpPrefix === '') {
            $httpPrefix = Yii::getAlias('@attachmentUrl') . '/';
        }

        return rtrim($httpPrefix, '/') . '/';
    }
