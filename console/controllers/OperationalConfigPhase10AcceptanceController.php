...
class OperationalConfigPhase10AcceptanceController extends Controller
{
    ...
    private function writeReport(string $result): string
    {
        ...
        $lines = array_merge($lines, [
            ...
            '```',
            '',
            'After browser/provider/production evidence is collected, rerun with:',
            '',
            '```bash',
            '/www/server/php/83/bin/php yii operational-config-phase10-acceptance/run \\',
            '  --baseUrl=https://demo2026.mongoyia.com \\',
            ...
            '  --providerEvidenceAccepted=1 --providerEvidencePath=<provider-evidence-report-or-ticket> \\',
            '  --productionEvidenceAccepted=1 --productionEvidencePath=<production-evidence-report-or-ticket> \\',
            '  --redactedExportAccepted=1 --redactedExportPath=<redacted-export-report> \\',
            '  --strict=1 \\',
            '  --interactive=0',
            '```',
            '',
            ...
            '',
            '1. Platform admin opens `/backend/mall/operational-config/index`.',
            '2. Payment cards show QPay, LianLian, and PayPal callback/return/cancel URL helpers and masked sensitive-field status.',
            '3. Save/check payment test-mode configs without exposing secrets in page text or export.',
            '4. Save SMTP config, run test-send, and confirm success/failure is recorded as a check row.',
            '5. Save alert recipients/triggers and run test-alert or record provider-unavailable failure clearly.',
