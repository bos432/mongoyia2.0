...
class CommonHelper
{
    ...
    public static function getStoreByHostName()
    {
        ...
        $defaultStore = $allStore[Yii::$app->params['defaultStoreId']] ?? null;
        if (in_array($hostName, self::platformHosts(), true) || in_array($hostName, self::legacyHostDomains(), true)) {
            return $defaultStore;
        }

        $storeId = $mapHostNameId[$hostName] ?? null;

        return $storeId ? $allStore[$storeId] : $defaultStore;
    }

    private static function platformHosts()
    {
        $hosts = [];
    ...
    private static function legacyHostDomains()
    {
        $domains = [];
    ...
    public static function getLanguage($store)
    {
        ...
        if ($lang && self::checkLang($lang) && self::checkStoreLang($store, $lang)) {
    ...
    public static function checkStoreLang($store, $lang)
    {
        $field = 'lang_' . str_replace('app-', '', Yii::$app->id);
        if (!isset($store->$field) || !$store->$field) {
            return true;
        }

        $langIds = Lang::getLanguageCode($store->$field, false, false, true);
        return in_array($lang, $langIds);
    }
