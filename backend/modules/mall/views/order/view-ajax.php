...
<div class="modal-header">
    ...
    <?php if ($isPlatformOperator): ?>
        <?= Html::a(Yii::t('app', 'Payment Attempts'), ['/mall/payment-attempt/index', 'ModelSearch' => ['order_id' => $paymentOrderId]], ['class' => 'btn btn-info btn-sm ml-3', 'target' => '_blank']) ?>
    <?php endif; ?>
...
<div class="modal-body order-view">
    ...
    <table class="table table-bordered table-hover">
        <tbody>
        <tr>
            <th style="width: 160px;"><?= Html::encode(Yii::t('app', 'Order Type')) ?></th>
            <td>
        ...
        <tr>
            ...
            <td>
                <?php if ($isPlatformOperator): ?>
                    <?= Html::a('#' . $paymentOrderId, ['view', 'id' => $paymentOrderId], ['target' => '_blank']) ?>
                <?php else: ?>
        ...
        <?php if ($parentOrder): ?>
            <tr>
                ...
                <td>
                    <?= Html::a('#' . $parentOrder->id, ['view', 'id' => $parentOrder->id], ['target' => '_blank']) ?>
    ...
    <?php if ($childOrders): ?>
        <table class="table table-bordered table-hover">
            ...
            <?php foreach ($childOrders as $childOrder): ?>
                <tr>
                    <td><?= Html::a('#' . $childOrder->id, ['view', 'id' => $childOrder->id], ['target' => '_blank']) ?></td>
    ...
    <?php if ($isPlatformOperator): ?>
        ...
        <table class="table table-bordered table-hover">
            ...
            <?php if ($paymentAttempts): ?>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <tr><td colspan="6" class="text-center text-muted"><?= Html::encode(Yii::t('app', 'No payment attempts yet')) ?></td></tr>
            <?php endif; ?>
            </tbody>
        </table>
    <?php endif; ?>
